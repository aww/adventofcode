...
parser = argparse.ArgumentParser(
    prog="boilerplate",
    description="""
    Sets up boilerplate code and an input file for a new day of Advent of Code.
...
if __name__ == "__main__":
    ...
    if args.label is None:
        ...
        r = requests.get(
            f"https://adventofcode.com/2025/day/{args.day}", cookies=cookies
        )
        bs = BeautifulSoup(r.text, "html.parser")
        assert bs.h2 is not None
        assert bs.h2.string is not None
        ...
        if re.match(r"^[_a-z]+$", args.label):
            print(
                style.GREEN + style.CHECK + style.RESET + f" using label {args.label}"
            )
        else:
            print(
                style.RED
                + f"X label {args.label} may need more sanitizing"
                + style.RESET
            )
            sys.exit(-1)
    ...
    fn_input = f"../private/2025/{full_label}_input.txt"
    ...
    r = requests.get(
        f"https://adventofcode.com/2025/day/{args.day}/input", cookies=cookies
    )
